...
public class Main extends Settings {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        if (System.getProperty("os.name").startsWith("Windows")) {
            ...
            final String selectedInterfaceID = InputUtils.getUserInput(
                    "[?] | Do you wanna enable ANSI support for Windows ?"
                            + "\n" +
            ...
            if (selectedInterfaceID.contains("y")) {
                ...
                System.out.println(
                        ColorsUtils.GREEN + "[v]" + ColorsUtils.RESET + " | ANSI Color support enabled!");
                ANSI = true;
            }
            else System.out.println(
                    "[v] | ANSI Color support disabled.");
        }
        ...
        System.out.println(
                ANSI ? ColorsUtils.PURPLE + "[-]" + ColorsUtils.RESET + " | Running network interfaces lookup..."
                        : "[-] | Running network interfaces lookup...");
        ...
        if (interfacesSize == 0) {
            System.out.println(
                    ANSI ? ColorsUtils.RED + "[x] " + ColorsUtils.RESET + " | No network interface detected, are they enabled?"
                            : "[x] | No network interface detected, are they enabled?");
            return;
        } else System.out.println(
                ANSI ? ColorsUtils.GREEN + "[v]" + ColorsUtils.RESET + " | Found " + interfacesSize + " network interfaces."
                        : "[v] | Found " + interfacesSize + " network interfaces.");
        ...
        final int interfaceID = Integer.parseInt(InputUtils.getUserInput(
        ...
        // Logging
        System.out.println(
                ANSI ? ColorsUtils.PURPLE + "[-]" + ColorsUtils.RESET + " | Running network scan on " + interfaceIP + ", please wait..."
                        : "[-] | Running network scan on " + interfaceIP + ", please wait...");
        ...
        for (final Map.Entry<String, String> set : scanResults.entrySet()) {
            System.out.println(
                    ANSI ? "    | " + (set.getValue().equals("N/A") ? ColorsUtils.RED + "[C] v<-x->? [H]": ColorsUtils.GREEN + "[C] v<--->v [H]") + ColorsUtils.RESET + " | #" + netID.size() + " | " + set.getKey() + (new String(new char[19 - set.getKey().length()]).replace("\0", " ")) + "| " + set.getValue() + " | #" + netID.size()
                            : "    | [C] v<--->v [H] | #" + netID.size() + " | " + set.getKey() + (new String(new char[19 - set.getKey().length()]).replace("\0", " ")) + "| " + set.getValue() + " | #" + netID.size());
        ...
        while (selectedNetID != -1) {
            selectedNetID = Integer.parseInt(InputUtils.getUserInput(
                    ANSI ? ColorsUtils.PURPLE + "[-]" + ColorsUtils.RESET + " | Please select the host you would like to identify by entering it's net ID:"
                            : "[-] | Please select the host you would like to identify by entering it's net ID:"));
